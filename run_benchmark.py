...
def train_benchmark_model(args, model_args, profile_name: str):
    ...
    if args.random:
        ...
        args.random_state = np.random.randint(0, 100000)
    ...
    summary(
        model.cpu(),
        input_size=model_args.summary_input,
        depth=9,
        verbose=1,
        col_names=[
            "input_size",
...
if __name__ == "__main__":
    ...
    wandb.init(
        project="t-jepa-2",
        reinit=True,
        name=f"{args.model_name}_{args.data_set}_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}",
        config=args,
    )

    val_metrics, test_metrics, args = train_benchmark_model(
        args, model_args, profile_name
    )
    ...
    profiling_info = []
    for func, (cc, nc, tt, ct, callers) in list(
        p.strip_dirs().sort_stats(SortKey.CUMULATIVE).stats.items()
    )[:10]:
        func_info = {
